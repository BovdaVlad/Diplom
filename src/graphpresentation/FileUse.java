...
public class FileUse {
    ...
    public void saveGraphNetAs(PetriNetsPanel panel, JFrame frame) throws ExceptionInvalidNetStructure {
        ...
        String name  = JOptionPane.showInputDialog(frame3,"Введіть назву системи","");
        ...
        if(name!=null) {
            ...
            try {
                ...
                fos = new FileOutputStream("src\\systems\\" + name + PATTERN);
                oos = new ObjectOutputStream(fos);

                JFrame frame2 = new JFrame("Оберіть картинку");
                FileDialog fdlg1;
                fdlg1 = new FileDialog(frame2, "Вибір іконки для системи", FileDialog.LOAD);
                fdlg1.setVisible(true);
                panel.getGraphNet().setImage((new File(fdlg1.getFile())));

                FileChannel source = null;
                FileChannel destination = null;
                try {
                    source = new FileInputStream(new File(fdlg1.getDirectory() + fdlg1.getFile())).getChannel();
                    destination = new FileOutputStream("src\\file_image\\" + fdlg1.getFile()).getChannel();
                ...
                panel.getGraphNet().createPetriNet(name);
            ...
            JOptionPane.showMessageDialog(frame1, "Модель системи збережено. Перезапустіть програму для її відображення");
        }
    }
    ...
    public GraphPetriNet generateGraphNetBySimpleNet(PetriNet net, Point paneCenter,File image) { // added by Katya 16.10.2016
        ...
        GraphPetriNet graphNet =  new GraphPetriNet(net, grPlaces, grTransitions, grArcIns, grArcOuts,image);
    ...
    public String openMethod(PetriNetsPanel panel, String methodFullName, JFrame frame) throws ExceptionInvalidNetStructure { // added by Katya 16.10.2016
        ...
        try {
            ...
            GraphPetriNet net = generateGraphNetBySimpleNet(convertMethodToPetriNet(methodText), paneCenter,null);
    ...
    public String saveNetAsMethod(GraphPetriNet pnet) throws ExceptionInvalidNetStructure {
        String s;
        PetriNet net;
        if (pnet.getPetriNet() == null) {
            pnet.createPetriNet("Untitled");
        ...
        net = pnet.getPetriNet();
        s="\n";
        s = s.concat("public static PetriNet CreateNet" + net.getName() + "(" + generateArgumentsString(net) + ") throws ExceptionInvalidNetStructure {\n" // modified by Katya 08.12.2016
                + "\t" + "ArrayList<PetriP> d_P = new ArrayList<>();\n"
                + "\t" + "ArrayList<PetriT> d_T = new ArrayList<>();\n"
                + "\t" + "ArrayList<ArcIn> d_In = new ArrayList<>();\n"
                + "\t" + "ArrayList<ArcOut> d_Out = new ArrayList<>();\n");
