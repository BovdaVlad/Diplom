...
public class PetriT extends PetriMainElement implements Cloneable, Serializable { // inheritance added by Katya 20.11.2016
    ...
    private ArrayList<Double> average_value = new ArrayList<>();
    private ArrayList<Double> average_time = new ArrayList<>();
    ...
    public void setTimeServ(double timeServ)
    {
        ...
        for (ArcIn arc: arcs) {
            if (arc.getNumT() == this.getNumber()) {
                if (arc.getIsInf() == true)
                {
                    ...
                    inP.add(arc.getNumP());
                    //System.out.println(name + "  " +arc.getNumP());
    ...
    public void actIn(PetriP[] pp, double currentTime) {
        if (this.condition(pp) == true) {
            for (int i = 0; i < inP.size(); i++) {
               ...
               // pp[inP.get(i)].addtime(parametr);
               // pp[inP.get(i)].addvalue(pp[inP.get(i)].getMark());
            }
            if (buffer == 0) {
    ...
    public void actOut(PetriP[] pp) {
        if (buffer > 0) {
            for (int j = 0; j < outP.size(); j++) {
                ...
                pp[outP.get(j)].addvalue(pp[outP.get(j)].getMark());
                //cc=parametr;
                pp[outP.get(j)].addtime(1); //getParamDeviation());
               // cc=parametr;
            }
    ...
    {
        average_value.add(value);
    }
    public void addtime(double time)
    {
        average_time.add(time);
    }

    public double getavarege_value(double timemodel)
    {
        double r =0;
        for(int i=0;i<average_value.size();i++)
        {
            r+=average_time.get(i)*average_value.get(i);
        }
        r = r/timemodel;
        return r;
    }
